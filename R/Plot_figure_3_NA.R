...
# Load data ---------------------------------------------------------------
q3 <- readRDS("outputs/models/revision/Q3_NA_fitted_model.RDs")
dc <- readRDS(file = "outputs/data/revision/Q3_NA_used_data.Rds")
dc_sp <- read_csv("outputs/tables/revision/Supp_species_average_success_NA.csv")
...
gq3c <- 
  ...
  labs(x = expression(paste("Aridity index")),
       y = "Species success"); gq3c

# Seed mass -----------------------------------------------------------------
preds <- as.data.frame(mnc6)
Xlabs <- unzt(formula = log(seedmass) ~ seedmassLZ, data = dc, xu = c(0.1, 1, 10, 100))
...
dc %>% 
  mutate(group = cut(aridity, breaks = c(-Inf, .1, .2, .4, .65))) -> dc2
table(dc2$group)
...
ggsave(
  plot = gq3,
  filename = "outputs/figures/revision/Figure_3_NA.png",
  width = 12,
  height = 8,
)
